...
***Note***: Since CMI-PB releases new data every year, the link used below may not be current. If this occurs, you will have to download the raw datasets from either the following [repository](https://github.com/jhsiao12/CMI-PB_Walkthrough/) or the CMI-PB website, under [Data and Resources tab](https://www.cmi-pb.org/blog/prediction-challenge-overview/#Data%20and%20resources).
...
***IMPORTANT!***: The advantage of using the tidymodels framework is that if we wanted to change the model to a different type of model, all we need to do is change the `linear_reg()` function to another function, such as Lasso regression, by changing only the arguments and `set_engine()` function as follows:
...
#   ab_validation <- validation_df %>%
#     mutate(predict(lm_train, validation_df))
...
actual_data <- fromJSON("https://www.cmi-pb.org/api/v4_1/plasma_ab_titer")
...
As you can see with our plotted predicted vs. actual 2022 data, and the Spearman Correlation, the model that was trained on 2021 data does very poorly on the 2022 data. This implies 'overfitting' - the model is too specific to the 2021 data and does not generalize well to the 2022 data. 

The lack of models that are able to reliably predict patient response to vaccines represents the goal of systems vaccinology. The very basic model we used in this tutorial is not sufficient to predict the immune response to the vaccine, and highlights how there is much to learn in how we can model complex biological responses. Understanding this will ultimately allow better vaccine design, and therefore elicit more robust individual and population immunity against preventable diseases.
...
To extend your learning, you may choose to look at other datasets in the CMI-PB challenge to see if you can model other types of data, such as PBMC cell frequency. These other datasets may be found [here](https://www.cmi-pb.org/blog/prediction-challenge-overview/#Data%20and%20resources).
